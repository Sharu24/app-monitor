...
checks.get = async (clientData) => {
  try {
    ...
    if (!istokenValid) {
      return Promise.resolve({
        statusCode: config.statusCodes.UNAUTHORIZED,
        Message: "UnAuthorised / Token Expired.",
      });
    }
    return Promise.resolve({ statusCode: config.statusCodes.SUCCESS, Message: checkData });
  } catch (err) {
    console.error(err);
    return Promise.reject({ statusCode: config.statusCodes.SERVER_ERROR, Message: "Server Error" });
  }
};
...
checks.post = async (clientData) => {
  try {
    ...
    return Promise.resolve({ statusCode: config.statusCodes.SUCCESS, Message: checkObject });
  } catch (err) {
    err;
    return Promise.reject({ statusCode: config.statusCodes.SERVER_ERROR, Message: "Server Error" });
  }
