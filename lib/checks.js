...
checks.get = async (clientData) => {
  try {
    ...
    if(!id) {
      return Promise.resolve({ "statusCode": 400, "Message": "Validation Failed/ Missing Fields" });
    }
    //Read check data 
    const checkData = await _data.read("checks", id);
    if(!checkData) {
      return Promise.resolve({ "statusCode": 404, "Message": "Check ID is not valid. Not Found." });
    }
    ...
    if (!istokenValid) {
      return Promise.resolve({ "statusCode": 401, "Message": "UnAuthorised / Token Expired." });
    }
...
checks.post = async (clientData) => {
  try {
    ...
    if(!userData) {
      return Promise.resolve({ "statusCode": 401, "Message": "UnAuthorised. No User Available with this token" });
    }
    const userChecks =
    ...
    if (userChecks.length > configs.maxChecks) {
      return Promise.resolve({ "statusCode": 401, "Message": `User already exhausted max checks. Limit is ${configs.maxChecks}` });
    }
...
checks.put = async (clientData) => {
  try {
    ...
    const checkData = await _data.read("checks", id);
    if(!checkData) {
      return Promise.resolve({ "statusCode": 400, "Message": "Check ID doesnt exist" });
    }
    //Check for access token
    let { token } = clientData.headers;
    token = typeof token === "string" ? token : false;
...
checks.delete = async (clientData) => {
  try {
    ...
    if(!userData) {
      return Promise.resolve({ "statusCode": 400, "Message": "Could not find the specified user"});
    }
    const userChecks =
    ...
    if (checkPosition > -1) {
      ...
      return Promise.resolve({ "statusCode": 200, "Message": "Deleted Check Id and Updated The User Schema" });
    }
  } catch (err) {
    console.log(err);
    return Promise.reject({ "statusCode": 500, "Message": "Server Error" });
