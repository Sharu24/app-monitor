...
workers.gatherAllChecks = () => {
    ...
    _data.list('checks', (err, checks) => {
        if (!err && checks && checks.length > 0) {
            ...
            console.log("There are no checks to perform at this moment " + new Date().toString());
        }
    })
}
...
workers.validateCheckData = (originalCheckData) => {
    ...
    if (originalCheckData.id &&
        ...
        console.error("Error : One of the checks file data is not properly formatted. Aborting the checks.");
    }
}
    ...
    _data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            ...
            if (alertWarranted) {
                ...
                console.log("Check Outcome has not changed. No need to alert user with sms/email");
            }
        } else {
            console.error("Error in updating the checks record. Check your APIs once.");
        }
    })
}
//Trigger the SMS Alert
workers.alertUserToStatusChange = (newCheckData) => {
    let msg = "Alert : Your check for " + newCheckData.method.toUpperCase() + ' ' + newCheckData.protocol
